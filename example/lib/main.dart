...
class _MyAppState extends State<MyApp> {
  ...
  void dispose() {
    super.dispose();
    _subscriptions.forEach((key, value) {(value as StreamSubscription).cancel();});
  }
  ...
  Future<void> _startSensor(int senID, List<double> data) async {
    if(!_subscriptions.containsKey(senID)){
      ...
      var tmpStream = stream.listen((sensorEvent) {
        BufferManager().fillBuffer(senID,SensorData(data[0],data[1],data[2]));
        setState(() {
          data.clear();
      ...
      home: Scaffold(
        ...
        body: Center(
          child: Column( children: [
            SensorInterface(sensorName: "Acc", isSensorAvailable:
             _accelAvailable, subFunction: _startSensor, id: Sensors.ACCELEROMETER, deSubFunction: _stopSensor),
            SensorInterface(sensorName: "Gyro",isSensorAvailable:
             _gyroAvailable, subFunction: _startSensor, id: Sensors.GYROSCOPE, deSubFunction: _stopSensor),
             MaterialButton(onPressed: () async { 
              if(BufferManager().isDatabaseReady()){
              var records = await BufferManager().readFromDatabase();
              for (var record in records){
                var data = SensorData(record.value['x'],record.value['y'],record.value['z']);
              }
              }
...
class SensorInterface extends StatefulWidget{
  ...
  final Function(int) deSubFunction;
    const SensorInterface({super.key, 
    required this.sensorName, 
    required this.isSensorAvailable, required this.subFunction, required this.id, required this.deSubFunction
    });
...
class _SensorInterfaceState extends State<SensorInterface> {
 List<double> data = <double>[0,0,0];
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text(widget.sensorName, style: const TextStyle(fontSize: 25, fontWeight: FontWeight.bold)),
        const Padding(padding: EdgeInsets.only(top: 16.0)),
        ValueBar(data: data),
        const Padding(padding: EdgeInsets.only(top: 16.0)),
        StartButton(data: data, subFunction: widget.subFunction, deSubFunction: widget.deSubFunction,
        isSensorAvailable: widget.isSensorAvailable, id: widget.id),
...
class StartButton extends StatefulWidget{
  ...
  final int id;
  final List<double> data;
  const StartButton({super.key, required this.subFunction, required this.deSubFunction, required this.id, 
  required this.data, required this.isSensorAvailable, });
...
class MedianButton extends StatefulWidget{
  ...
  final int id;
  const MedianButton({super.key, required this.sensorDataMedian, required this.id});
